...
app.listen(PORT, () => {
  const url = BASE_URL ? `http://localhost:${PORT}${BASE_URL}` : `http://localhost:${PORT}`;
  console.log(`Setlists Manager running at ${url}`);
  ...
  if (userCount === 0) {
    console.log(`⚠️  No hay usuarios configurados. Visita ${url}/setup para crear el administrador.`);
  }
});